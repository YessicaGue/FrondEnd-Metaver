...
class InicioCCController extends Controller
{
    public function index(): \Inertia\Response {
        ...
        $request = $client->request('GET',env('API_BASE')."/api/candidatosCHC/validarCandidato/{$perfilId}");
        $estatusEtapa = json_decode($request->getBody()->getContents());
        if (!is_null($perfilId) && $perfilId == 400) // JG - 20230828 - Salto de validaciones
            return Inertia::render('CaminoCandidato/InicioCC', [ // JG - 20230828 - Salto de validaciones
                'isDemo' => true, // JG - 20230828 - Salto de validaciones
                "auth" => [ "user" => $user ], //JJ 20230912
                "estatusEtapa" => (object)[]
            ]); // JG - 20230828 - Salto de validaciones
        return Inertia::render('CaminoCandidato/InicioCC',
            [
                "auth" => [ "user" => $user ], //JJ 20230912
