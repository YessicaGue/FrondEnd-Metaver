...
class CartaEvidenciaController extends Controller
{
    ...
    public function index(): \Inertia\Response {
        ...
        $client = new Client(['verify' => false]);
        $request = $client->request('GET', env('API_BASE')."/api/cartaEvidencia/get-carta-evidencia/{$perfilId}");
        $cartaEvidencia = json_decode($request->getBody()->getContents());

        return Inertia::render('CaminoCandidato/CartasEvidencias', [
            'cartaEvidencia' => $cartaEvidencia -> response
        ]);
    ...
    public function editPDFCartaEvidencia(Request $request)
    {
...
//            return($uuid);
        ...
        $archivo = $request -> file('carta_evidencia', );
        if(!$archivo){
        ...
        if (!in_array($extension, ['pdf', 'png', 'jpg', 'docx', 'doc', 'jpeg'])) {
            return response()->json(['error' => 'El archivo no es valido'], 400);
        }

        $descripcionArchivoUsuario = $request->input('descripcion_archivo_usuario');
        $nombreArchivoUsuario = $request->input('nombre_archivo_usuario');

        $result = $request->file('carta_evidencia')->store('candidatos_chc/'.$uuid);
        ...
        $body = [
            'multipart' => [
                ...
                [
                    ...
                    'contents' => $nombreArchivoUsuario,
                ],
                [
                    'name' => 'descripcion_archivo_usuario',
                    'contents' => $descripcionArchivoUsuario,
                ]
            ],
        ];
        try {
            $request = $client->request('POST', env('API_BASE') . "/api/cartaEvidencia/post-carta-evidencia", $body);
            ...
            // return response()->json([
            //     'nombreArchivo' =>  $nombreArchivoUsuario,
            //     'descripcionArchivo' =>  $descripcionArchivoUsuario,
            // ]);
        }
