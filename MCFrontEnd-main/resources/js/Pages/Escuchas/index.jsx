...
import { TextField, FormControlLabel, Checkbox, Button, Link, Typography } from '@mui/material';
...
import headImage from './banner_movimiento.png';
import PruebaLabel from './pruebalabel';
import './style.css';
import '../../../assets/fonts/CompactaSHOP-Light.woff';
import '../../../assets/fonts/Anton-Regular_1.woff';
import linea_banner from './linea_banner.png';


// ...


import ParticlesConfetti from '@/Components/Customized/ParticlesComponents/ParticlesConfetti';

const Footer = () => {
    return (
        <div className='w-full text-white py-12 bg-[#101010]'>
            <div className='max-w-7xl mx-auto px-6 xs:px-6 sm:px-10 lg:px-30 h-full flex flex-col'>
                ...
                <div className='pt-4'>
                    <h1 className='text-xl'>Teléfono:</h1>
                    <p>222 333 4567</p>
                </div>
                <div className='pt-4'>
...
const index = () => {
    ...
    const [estaHecho, setEstaHecho] = useState(false);
    ...
    const handleSubmit = async () => {
        if (respuesta.length <= 0)
            return Swal.fire({
                title: 'Lo sentimos',
                text: 'No has escrito comentario alguno. Por favor, inténtalo de nuevo',
                icon: 'info'
            });
        ...
        const { data: response } = await axios.post(route('post.movimiento.escucha'), formData);

        if (!response.success)
            return Swal.fire({
                title: 'Lo sentimos',
                text: 'Tu comentarios no fueron enviados. Por favor, inténtalo más tarde',
                icon: 'info'
            });

        await Swal.fire({
            title: 'Enhorabuena',
            text: 'Tu comentarios han sido enviados',
            icon: 'success'
        });

        const localStorageRes = await (() => {
            return new Promise((resolve, reject) => {
                try {
                    resolve(localStorage.setItem('movestoken', JSON.stringify(response.response)));
                } catch (err) {
                    reject(false);
                }
            })
        ...
        if (localStorageRes === false)
            return Swal.fire({
                title: 'Lo sentimos',
                text: 'Aunque tus comentarios fueron enviados, algo salió mal para poder brindarte más información. Por favor, inténtalo de nuevo',
                icon: 'info'
            });
    ...
    return (
        <div className='w-full'>
            { estaHecho && <ParticlesConfetti /> }
            <Head title="El Movimiento Escucha"/>

            <div className='bg-orange-400 w-full h-[650px] pt-20 text-center justify-center item-center rounded-bl-3xl'>
                <span className='text-white text-9xl w-full xs:text-xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl tracking-wider' style={{ fontFamily: 'CompactaSHOP', letterSpacing: '2px', fontweigh:'800' }}>MOVIMIENTO </span>
                <span className='text-white text-9xl w-full xs:text-xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl' style={{ fontFamily: 'Anton'}}> <br /> ESCUCHA</span>
                <img className='mx-auto' src={ linea_banner } />
            </div>
            

            <div className='max-w-7xl mx-auto px-6 xs:px-6 sm:px-10 lg:px-30 flex flex-col gap-8 py-12 items-center'>
                ...
                <TrailAppear>

                    {
                        ...
                        ?   <>
                                <TrailAppear>
                                    ...
                                    <div className='max-w-lg mx-auto flex flex-col gap-4'>
                                        <div className='font-semibold text-lg text-center' style={{ fontFamily: 'Poppins, sans-serif' }}>
                                            Gracias por compartirnos tus comentarios. ✨
                                        </div>
    
                                        <div className='text-sm mt-1 text-center' style={{ fontFamily: 'Poppins, sans-serif' }}>
                                            Hemos recibido tu opinión. Estaremos siempre listos para escucharte.
                                        </div>
                                    </div>
                                    ...
                                    <div className='w-auto mt-4'>
                                        <Button 
                                            variant="contained" 
                                            onClick={ () => location.replace(route('participar.me.page')) } 
                                            style={{ fontFamily: 'Poppins, sans-serif' }}
                                        >
                                            Quiero mantenerme informad@ de Movimiento Ciudadano
                                        </Button> 
                                    </div>
                        ...
                        :   <>
                                ...
                                <div className='text-7xl w-full text-center text-orange-500 ' style={{ fontFamily: 'CompactaSHOP', letterSpacing: '2px', fontweigh:'500' }}>
                                    ¡QUEREMOS ESCUCHARTE!
                                </div>
                                    <div className='w-full text-center mt-4 text-lg font-bold tracking-widest'>
                                        Escríbenos tus comentarios para que podamos escucharte
                                    </div>
                                ...
                                <div className='flex flex-col gap-16'>
                                    ...
                                    <div className='flex flex-col gap-4'>
                                        ...
                                        <textarea className="mb-4 shadow-xl rounded-xl"
                                            multiline
                                            rows={4}
                                            color="primary"
                                            value={ respuesta }
                                        ...
                                        <div className='grid gap-2 max-lg:grid-cols-4 grid-cols-6 max-lg:[&>*:first-child]:col-span-3 [&>*:first-child]:col-span-5'>
                                            <div></div>
